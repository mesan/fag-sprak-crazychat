...
func MessageObserverAsGenericObserver(observer MessageObserver) GenericObserver {
	return NewGenericObserverFunc(func(next interface{}, err error, complete bool) {
		...
		case complete:
			observer.Complete()
		default:
			observer.Next(next.(Message))
		}
	})
}

func GenericObserverAsMessageObserver(observer GenericObserver) MessageObserver {
	return MessageObserverFunc(func(next Message, err error, complete bool) {
		switch {
		case err != nil:
			observer.Error(err)
		case complete:
...
func RepeatMessage(value Message, count int) *MessageStream {
...
// otherwise it will be a single-value stream of Message.
func StartMessage(f func() (Message, error)) *MessageStream {
	return CreateMessage(func(observer MessageObserver, subscription Subscription) {
		if v, err := f(); err != nil {
			observer.Error(err)
		} else {
...
func PassthroughMessage(next Message, err error, complete bool, observer MessageObserver) {
	switch {
	case err != nil:
		observer.Error(err)
	case complete:
...
var zeroMessage = *new(Message)

type MessageObserverFunc func(Message, error, bool)

func (f MessageObserverFunc) Next(next Message) { f(next, nil, false) }
...
func (f GenericObservableFilterFactory) Message(parent MessageObservable) MessageObservable {
	return MapMessage2MessageObservable(parent, func(observer MessageObserver) MappingMessage2MessageFunc {
		...
		filter := f(gobserver)
		return func(next Message, err error, complete bool, observer MessageObserver) {
...
func FromMessageArray(array []Message) *MessageStream {
	return CreateMessage(func(observer MessageObserver, subscription Subscription) {
		for _, v := range array {
			if subscription.Disposed() {
				return
...
func FromMessages(array ...Message) *MessageStream {
	return FromMessageArray(array)
}

func JustMessage(element Message) *MessageStream {
	return FromMessageArray([]Message{element})
}
...
func FromMessageChannel(ch <-chan Message) *MessageStream {
	return CreateMessage(func(observer MessageObserver, subscription Subscription) {
		for v := range ch {
			if subscription.Disposed() {
				return
...
func (s *MessageStream) SubscribeFunc(f func(Message, error, bool)) Subscription {
	return s.Subscribe(MessageObserverFunc(f))
}

func (s *MessageStream) SubscribeNext(f func(v Message)) Subscription {
	return s.SubscribeFunc(func(next Message, err error, complete bool) {
		if err == nil && !complete {
			f(next)
		}
	})
...
func (s *MessageStream) Filter(f func(Message) bool) *MessageStream {
	return FromMessageObservable(filterFilter(func(v interface{}) bool { return f(v.(Message)) }).Message(s))
}
...
func (s *MessageStream) Wait() error {
	...
	s.SubscribeFunc(func(next Message, err error, complete bool) {
		switch {
		case err != nil:
			errch <- err
		case complete:
...
func (c *concatMessageSubscriber) Next(next Message) {
...
func (m *mergeMessageObservable) Subscribe(observer MessageObserver) Subscription {
	...
	relay := func(next Message, err error, complete bool) {
		lock.Lock()
		defer lock.Unlock()
...
func (r *catchMessageObservable) Subscribe(observer MessageObserver) Subscription {
	subscription := NewGenericSubscription()
	run := func(next Message, err error, complete bool) {
		switch {
		case err != nil:
...
func (r *retryMessageObserver) retry(next Message, err error, complete bool) {
	switch {
	case err != nil:
...
func (s *MessageStream) Do(f func(next Message)) *MessageStream {
	return FromMessageObservable(MapMessage2MessageObserveNext(s, func(next Message) Message {
		f(next)
		return next
	}))
}
...
func (s *MessageStream) DoOnError(f func(err error)) *MessageStream {
	return FromMessageObservable(MapMessage2MessageObserveDirect(s, func(next Message, err error, complete bool, observer MessageObserver) {
		if err != nil {
			f(err)
		}
...
func (s *MessageStream) Reduce(initial Message, reducer func(Message, Message) Message) *MessageStream {
	value := initial
	return FromMessageObservable(MapMessage2MessageObserveDirect(s, func(next Message, err error, complete bool, observer MessageObserver) {
		switch {
		case err != nil:
...
func (s *MessageStream) Scan(initial Message, f func(Message, Message) Message) *MessageStream {
	value := initial
	return FromMessageObservable(MapMessage2MessageObserveDirect(s, func(next Message, err error, complete bool, observer MessageObserver) {
		switch {
		case err != nil:
...
func (s *MessageStream) Fork() *MessageStream {
	...
	go s.Subscribe(MessageObserverFunc(func(n Message, err error, complete bool) {
		f.lock.Lock()
		defer f.lock.Unlock()
...
func (s *MessageStream) ToOneWithError() (Message, error) {
	valuech := make(chan Message, 1)
	errch := make(chan error, 1)
	FromMessageObservable(oneFilter().Message(s)).SubscribeFunc(func(next Message, err error, complete bool) {
		if err != nil {
			errch <- err
		} else if !complete {
			valuech <- next
...
func (s *MessageStream) ToOne() Message {
...
func (s *MessageStream) ToArrayWithError() ([]Message, error) {
	array := []Message{}
...
func (s *MessageStream) ToArray() []Message {
...
func (s *MessageStream) ToChannelWithError() (<-chan Message, <-chan error) {
...
func (s *MessageStream) ToChannel() <-chan Message {
...
func (s *MessageStream) Count() *IntStream {
	count := 0
	return FromIntObservable(MapMessage2IntObserveDirect(s, func(next Message, err error, complete bool, observer IntObserver) {
		switch {
		case err != nil:
			observer.Next(count)
			observer.Error(err)
...
type MappingMessage2MessageFunc func(next Message, err error, complete bool, observer MessageObserver)
...
func (f *MappingMessage2MessageObservable) Subscribe(observer MessageObserver) Subscription {
	...
	return f.parent.Subscribe(MessageObserverFunc(func(next Message, err error, complete bool) {
...
func MapMessage2MessageObserveNext(parent MessageObservable, mapper func(Message) Message) MessageObservable {
	return MapMessage2MessageObservable(parent, func(MessageObserver) MappingMessage2MessageFunc {
		return func(next Message, err error, complete bool, observer MessageObserver) {
			var mapped Message
			if err == nil && !complete {
...
type flatMapMessage2Message struct {
	parent MessageObservable
	mapper func(Message) MessageObservable
}

func (f *flatMapMessage2Message) Subscribe(observer MessageObserver) Subscription {
	subscription := NewGenericSubscription()
	wg := sync.WaitGroup{}
	f.parent.Subscribe(MessageObserverFunc(func(next Message, err error, complete bool) {
		switch {
		case err != nil:
			wg.Wait()
			observer.Error(err)
...
func (s *MessageStream) Map(f func(Message) Message) *MessageStream {
	return FromMessageObservable(MapMessage2MessageObserveNext(s, f))
}

func (s *MessageStream) FlatMap(f func(Message) MessageObservable) *MessageStream {
	return &MessageStream{&flatMapMessage2Message{s, f}}
}

type MappingMessage2IntFunc func(next Message, err error, complete bool, observer IntObserver)
...
func MapMessage2IntObserveNext(parent MessageObservable, mapper func(Message) int) IntObservable {
	return MapMessage2IntObservable(parent, func(IntObserver) MappingMessage2IntFunc {
		return func(next Message, err error, complete bool, observer IntObserver) {
			var mapped int
			if err == nil && !complete {
				mapped = mapper(next)
			}
...
type flatMapMessage2Int struct {
	parent MessageObservable
	mapper func(Message) IntObservable
}
...
func (s *MessageStream) MapInt(f func(Message) int) *IntStream {
	return FromIntObservable(MapMessage2IntObserveNext(s, f))
}

func (s *MessageStream) FlatMapInt(f func(Message) IntObservable) *IntStream {
...
func MapInt2MessageObserveNext(parent IntObservable, mapper func(int) Message) MessageObservable {
	return MapInt2MessageObservable(parent, func(MessageObserver) MappingInt2MessageFunc {
...
func (s *IntStream) MapMessage(f func(int) Message) *MessageStream {
